...
def generate_report(
    ...
    lines.append("├─────────────────────────────────────────────────────────────────┤")
    lines.append(f"│  [填] 应纳税所得额 (方案A): {s['hk_gains_taxable'] + s['us_gains_taxable']:>10.2f} CNY           │")
    lines.append(f"│  [填] 应纳税所得额 (方案B): {s['total_gains_taxable']:>10.2f} CNY           │")
    lines.append(f"│       应纳税额     (方案A): {capital_tax_a:>10.2f} CNY (系统自算)  │")
    lines.append(f"│       应纳税额     (方案B): {capital_tax_b:>10.2f} CNY (系统自算)  │")
    lines.append(f"│  [填] 境外已缴税额:               0.00 CNY (无预扣税)   │")
    lines.append("└─────────────────────────────────────────────────────────────────┘")
    lines.append("")
    lines.append("┌─────────────────────────────────────────────────────────────────┐")
    lines.append("│ 栏目：利息、股息、红利所得                                      │")
    lines.append("├─────────────────────────────────────────────────────────────────┤")
    lines.append(f"│  [填] 应纳税所得额:    {s['total_dividend_cny']:>12.2f} CNY (分红总额)     │")
    lines.append(f"│       应纳税额:        {dividend_tax_gross:>12.2f} CNY (系统自算)     │")
    lines.append(f"│  [填] 境外所得境外支付部分预缴税额: {s['foreign_tax_credit_cny']:>5.2f} CNY (美股预扣)│")
    lines.append(f"│  [填] 境外所得境内支付部分已缴税额: {s['hshare_tax_credit']:>5.2f} CNY (H股代扣)│")
